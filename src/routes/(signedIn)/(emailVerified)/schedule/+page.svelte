...
<div class="grid md:grid-cols-3">
  ...
  <div class="col-span-2 mt-32">
    <div class="calendar-container">
      ...
      <!-- does the calculations so that the event is at the correct part of the grid -->
      <div class="event-container border border-gray-200 shadow">
        ...
        {#each $ScheduleStore as event (event.id)}
          <div
            ...
            class="slot"
            style={`height: ${
              (convertInputTime(event.end) - convertInputTime(event.start)) * 15
            }px; grid-row: ${convertInputTime(
              event.start,
            )}; grid-column: ${convertInputDate(event.date)}; width:${calWidth(
              convertInputDate(event.date),
            )}%; background-color:${eventColor(event.category)} `}
          ...
          >
            ...
            <div
              ...
              style={`margin-top: ${
                (convertInputTime(event.end) - convertInputTime(event.start)) *
                  15 ===
                30
                  ? '.05px'
                  : '2px'
      ...
      <div class="modal">
        <div class="modal-content rounded-md">
          <div class="mb-5 ml-5 mt-5">
            ...
            <button
              class="mt-5 rounded-md bg-blue-100 px-4 py-2 text-blue-900 shadow-sm transition-colors duration-300 hover:bg-blue-200"
              on:click={closeModal}>Close Modal</button
            >
          </div>
        </div>
      </div>
